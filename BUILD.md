...
- Установка **per-user**, без прав администратора: `PrivilegesRequired=lowest`,
  каталог `%LOCALAPPDATA%\Programs\OptiCIP Dashboard`. Благодаря этому тихое
  автообновление не вызывает UAC.
- Данные, кэш и логи приложение хранит в `%LOCALAPPDATA%\OptiCIP Dashboard`
  (постоянный пользовательский корень, не зависит от каталога установки — при
  переезде с прежней установки в `Program Files` ничего не теряется).
