      ...
      labeledSegments.forEach((segment) => {
        ...
        svg.append(
          createSvgNode("line", {
            ...
            y2: plotTop - 8,
            // Выноска нейтральная, а не цветом фазы: она указывает на полосу и
            // обязана дожить до ч/б печати, где цвет фазы выцветает в белый.
            stroke: SEGMENT_EDGE_STROKE,
            "stroke-width": 1,
            opacity: 0.75,
          })
        );
      ...
      panelLayouts.forEach((layout) => {
        ...
        labeledSegments.forEach((segment, index) => {
          ...
          const edges = index === labeledSegments.length - 1
            ...
            : [segment.startX];
          edges.forEach((edgeX) => {
            // Стык у самого края поля совпал бы с осью или рамкой панели.
            if (
              edgeX <= chartLayout.left + 1 ||
              edgeX >= chartLayout.width - chartLayout.right - 1
            ) {
              return;
            }
            svg.append(
