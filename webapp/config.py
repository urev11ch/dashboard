...
DEFAULT_APP_SETTINGS: dict[str, Any] = {
    ...
    # Обновляться при запуске без единого нажатия: проверка → скачивание →
    # тихая установка → перезапуск. Один запрос UAC остаётся — установщик кладёт
    # приложение в Program Files, и обойти это можно только per-user установкой.
...
# Потолок строк в одной выгрузке журнала в .xlsx. Лист Excel держит ~1 048 575
# строк данных, но каждая строка выгрузки читает сэмплы своей мойки — на
# несколько десятков тысяч запрос уже идёт минутами, и честнее отказать с
# внятным текстом, чем подвесить окно.
WASH_EXPORT_MAX_ROWS = 20000
...
# ПК инженера нередко стоит сразу в нескольких заводских подсетях (напр.
# 192.168.18.x и 192.168.19.x) — сканируем /24 каждого интерфейса, но не больше
# этого числа сетей: каждая — это 254 TCP-пробы.
FTP_DISCOVERY_MAX_NETWORKS = 4
