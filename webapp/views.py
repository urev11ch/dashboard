...
def capture_state_snapshot() -> AppStateSnapshot:
    return AppStateSnapshot(
        ...
        program_name_overrides=dict(state.program_name_overrides),
        error=state.error,
...
def build_program_rows(
    ...
    overrides = overrides or {}
    seen = program_ids_seen(analysis)
    ...
    program_ids |= {program_id for program_id in overrides if program_id > 0}

    rows: list[dict[str, Any]] = []
    for program_id in sorted(program_ids):
        base_name = core.fallback_program_name(program_id)
        own_name = overrides.get(program_id, "")
        rows.append(
            {
                "program_id": program_id,
                "program_name": own_name or base_name,
                "own_name": own_name,
