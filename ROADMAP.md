...
3. ~~**Per-user установка + фоновое обновление**~~ — **сделано** (1.1.32):
   `PrivilegesRequired=lowest`, `%LOCALAPPDATA%\Programs`, миграция сносит
   прежнюю админскую копию через `PrepareToInstall`. Цикл проверка→скачивание→
   установка→перезапуск идёт молча. **Требует проверки на живой Windows:**
   переход с админской копии (один UAC на удаление), тихое обновление без UAC,
   перезапуск, автозапуск после переезда.
