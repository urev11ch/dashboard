...
; Restart Manager закрывает приложения, которые держат заменяемые файлы.
; force — потому что закрыться умеет не всякий процесс: приложение без единого
; видимого окна (webview не вернул управление) на WM_CLOSE не реагирует, и
; мастер останавливался на «программе установки не удалось закрыть все
; приложения». Такой процесс завершается принудительно — терять ему нечего:
; настройки и кэш пишутся атомарно.
CloseApplications=force
; Перезапуск после установки делает сама программа установки ([Run] с
; /RELAUNCH=1 при тихом обновлении), поэтому Restart Manager поднимать
; приложение не должен — иначе после обновления открылись бы два окна.
