...
3. **Per-user установка + фоновое обновление** — *теперь это единственное, что
   осталось для «совсем без подтверждений»: цикл проверка→скачивание→установка→
   перезапуск уже автоматический (`auto_update_enabled`), но один запрос UAC
   неизбежен, пока приложение живёт в Program Files.* (вернуть `fc1bb53`, доработать). Тест на живой Windows: тихая установка без UAC, миграция/сосуществование со старой Program Files-копией, перезапуск. Явно решить миграцию (оставлять старую копию для отката vs. снимать).

## P1 — Аналитика мойки
4. **Пороги по температуре и времени выдержки фаз** (сейчас только концентрация).
5. **Нормативы по программам/объектам**, а не глобально. Ключ области уже есть —
   `core.program_scope_key/parse_program_scope_key/program_scope_chain` («*» →
   канал → объект), сделан для названий программ. Нормативы должны взять его же,
   а не заводить вторую схему скоупа: иначе две схемы разойдутся.
6. **Экспорт** журнала: выгрузка в `.xlsx` — **готово** (кнопка «Выгрузить XLSX»,
   `POST /api/wash-export`, колонки по фазам щёлочи/кислоты). Остаются CSV и
   пакетный PDF по периоду.
